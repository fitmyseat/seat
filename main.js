    ...
    AppComponent.prototype.mapCardsFromCsv = function (csvText) {
        ...
        return rows.slice(1)
            ...
            var description = headers
                ...
                .filter(function (item) { return item.header !== 'Model' && item.header !== 'Qty' && item.header !== 'Quantity' && item.header !== 'Image' && item.header !== 'ImageName' && item.header !== 'ImageFile'; })
                .map(function (item) { return item.header + ': ' + item.value; })
                .join(' | ');
            return {
                model: model,
                year: year,
